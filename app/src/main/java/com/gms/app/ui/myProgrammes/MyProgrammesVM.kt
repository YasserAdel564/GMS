...
class MyProgrammesVM @ViewModelInject
constructor(
    private val myProgramsRepo: MyProgramsRepo,
) : BaseViewModel() {
    ...
    val uiState: LiveData<Event<UiStates>>
        get() = _uiState
    var myProgramsList: ArrayList<MyProgrammeModel> = arrayListOf()
    ...
    private fun launchJob(): Job {
        ...
        return CoroutineScope(Dispatchers.Main).launch {
            when (val response = myProgramsRepo.getMyPrograms()) {
                is DataResource.Success -> {
                    myProgramsList.clear()
                    myProgramsList.addAll(response.value)
